...
falsifiable_if: "the vault has more orphan islands after dreaming than before, or Sean reports the vault is still disorganized"
applies_to:
  - vault-maintenance
...
- Sean asks to "organize," "prune," "consolidate," or "dream"
...
for f in sorted(os.listdir(vault_root)):
    if journal_pattern.match(f):
        ...
        if f == today:
            ...
            # Keep today's even if empty — Sean might still write
            continue
        if content:
            journal_files.append({"file": f, "content": content, "chars": len(content)})
        else:
...
For each non-empty journal entry: read it, extract topics, research them with `vault_research`, write linked notes that link TO the journal entry via `[[2026-07-25]]`. The journal is Sean's raw thought signal — treat it as gold. NEVER modify or delete non-empty journals.
...
if total_gaps == 0:
    ...
    prompt_parts.extend([
        ...
        "",
        "## Calibration Gaps (Sean's corrections — where VaultBot failed)",
        "These are places where Sean corrected VaultBot. Each is a failure pattern.",
...
Rules: Always back up before deleting (vault_delete does this automatically). Never delete sacred journals, LOCKED notes, or identity files. When in doubt, don't delete — flag for Sean.
...
- **Hub notes** must exist: Testing-and-Verification-History, VaultBot-Build-Log, Sean-Design-Decisions, Cross-Session-Patterns-from-75-Chat-Logs, Sean-Communication-Preferences
...
- **Light dream** (Steps 1-2, 4-5 only): Every 10-15 new chat logs or when Sean asks. Just connects orphans and prunes junk. ~5 minutes.
...
- [[Sean-Design-Decisions]] — design decisions hub
