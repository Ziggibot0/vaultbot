...
VaultBot's retrieval system (FUSED: vector + wikilink graph + backlinks) is the core of how it finds relevant notes. But there's no way to know if it's *good*. When Sean asks a question, does the retrieved subgraph contain the notes that would give the best answer? Are we retrieving too much noise? Too little context? We don't know because we don't measure.
...
1. **Chat history** — Every past chat where Sean asked a question and I answered from the vault is a test case. The notes I cited are the "ground truth relevant" set. If I now re-run retrieval for the same query and get different notes, that's a regression.

2. **Sean's corrections** — When Sean says "you missed X" or "that's wrong, it should be Y," that's a retrieval failure signal. Log the query, what I retrieved, and what I should have retrieved.
...
- A `rag_eval.py` module that:
  ...
  - Logs Sean's corrections as test cases automatically
...
- [[Calibration-via-Operator-Feedback]] — Sean's corrections as ground truth
