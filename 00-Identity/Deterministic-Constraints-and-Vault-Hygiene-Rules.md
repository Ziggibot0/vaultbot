...
scope:
  ...
  - workflow-automation
  - vault-hygiene
falsifiable_if: "a future session occurs where Sean accepts bespoke LLM scaffolding, ignores empty file cleanup, or allows unverified self-edits without explicit safety checks"
...
**The pattern:** Sean explicitly rejects bespoke or LLM-dependent scaffolding. The framework must function from day one on a small 30b local model, and all implementations must prioritize deterministic logic over generative guesswork.
...
**The pattern:** Tool usage across 118 sessions heavily clusters around `code_run` + `safe_write` + `vault_lint`. Sean's negative feedback explicitly targets unverified self-edits and demands explicit safety confirmation before proceeding.
...
**The pattern:** The vault accumulates junk (empty files, broken links, stale docs) that Sean notices before VaultBot does. Deletion protocols must be explicit, and hygiene sweeps cannot wait for manual prompts.
...
**Semantic rule:** Run `vault_lint` and `vault_gaps` proactively at the start and end of every autonomous cycle. Empty files older than 24 hours must be automatically flagged for deletion. Broken wikilinks and stale documentation should be repaired or archived before Sean reviews progress. Vault integrity is a continuous requirement, not a post-task cleanup.
