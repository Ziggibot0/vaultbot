...
scope:
  ...
  - vault-hygiene
  - operator-trust
falsifiable_if: "a future session occurs where Sean explicitly trusts ephemeral memory or stops demanding explicit progress persistence checks before proceeding"
...
**The pattern:** Sean explicitly distrusts ephemeral memory and demands that VaultBot persist all state changes to the vault before proceeding or claiming completion. He treats un-saved progress as non-existent and refuses to trust verbal assurances from an LLM with temporary context.

**Evidence:**
- "i will not trust any promise from an LLM with ephemeral memory" — Sean explicitly rejects ephemeral state as a valid basis for task continuity
- "have you saved your progress and updated all your notes?" — Sean demands explicit verification that in-memory work has been persisted to disk before accepting completion
- "what's next is you update yourself so you know where you stand and aren't j chillin with stale-ass info" — Sean proactively enforces self-model sync as a persistence requirement, not an optional step
