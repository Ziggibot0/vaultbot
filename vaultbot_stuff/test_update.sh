...
mkdir -p "$VAULT_PATH/vaultbot_stuff/Memory/Chat"
echo "fake chat log" > "$VAULT_PATH/vaultbot_stuff/Memory/Chat/test-chat.md"

mkdir -p "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/sessions"
echo '{"session":"fake"}' > "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/sessions/test.jsonl"

mkdir -p "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/identity"
echo "# My identity" > "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/identity/IDENTITY.md"

# Create a fake custom tool (should survive update — copyCodeTree doesn't delete)
echo "# My custom tool" > "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/custom_tools/my_custom_tool.py"
...
# The plugin's copyCodeTree copies from archiveRoot/vaultbot_stuff/vaultbot_backend/
# to the live backend dir. It does NOT delete files that exist in dest but not src.
SRC_BACKEND="$ARCHIVE_ROOT/vaultbot_stuff/vaultbot_backend"
DST_BACKEND="$VAULT_PATH/vaultbot_stuff/vaultbot_backend"

if [ ! -d "$SRC_BACKEND" ]; then
    fail "Archive has no vaultbot_stuff/vaultbot_backend/"
    exit 1
fi
...
check_survived "$VAULT_PATH/vaultbot_stuff/Memory/Chat/test-chat.md"
check_survived "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/sessions/test.jsonl"
check_survived "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/identity/IDENTITY.md"
check_survived "$VAULT_PATH/vaultbot_stuff/vaultbot_backend/custom_tools/my_custom_tool.py"
...
mkdir -p "$VAULT_PATH/vaultbot_stuff/Memory/Chat"
mkdir -p "$VAULT_PATH/vaultbot_stuff/Memory/Build-Log"
mkdir -p "$VAULT_PATH/vaultbot_stuff/Knowledge/Research"
cd "$VAULT_PATH"
