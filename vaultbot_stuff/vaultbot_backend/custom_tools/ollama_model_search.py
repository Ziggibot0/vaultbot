...
SCHEMA = {"name": "ollama_model_search", "description": "Search Ollama's model library (https://ollama.com/search), list available tags for a model, list installed models, or pull a model. Actions: 'search' (query the web library), 'tags' (get available tags for a model), 'installed' (list locally installed models), 'pull' (pull a model via ollama pull).", "parameters": {"properties": {"action": {"description": "One of: search, tags, installed, pull", "type": "string"}, "category": {"description": "Category filter for search (e.g. vision, tools, embedding, reasoning). Optional.", "type": "string"}, "query": {"description": "Search query (for action=search) or model name (for action=tags or action=pull)", "type": "string"}, "tag": {"description": "Specific tag to pull (for action=pull). If omitted, pulls :latest.", "type": "string"}}, "required": ["action"], "type": "object"}}
...
def _parse_search_results(html: str) -> list:
    ...
    for block in blocks[1:]:  # skip first (before first <li>)
        ...
        # Extract description
        desc_match = re.search(r'<p\s+class="max-w-lg[^"]*"[^>]*>(.*?)</p>', block, re.DOTALL)
        description = ""
        if desc_match:
...
def run(args: dict) -> dict:
    ...
    if action == "search":
        ...
        if not model:
            return {"status": "error", "message": "Model name required (pass via 'query' or 'model')"}
        ...
        return {"status": "error", "message": f"Unknown action: {action}. Use: search, tags, installed, pull"}
