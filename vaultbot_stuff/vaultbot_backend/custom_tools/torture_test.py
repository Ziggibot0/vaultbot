...
SCHEMA = {"name": "torture_test", "description": "Run torture tests on a pull request before merging. Downloads changed files from the PR branch and runs: Python syntax check, JS syntax check, .gitignore tampering check, malware/exfiltration pattern scan, path whitelist check. Returns a structured pass/fail report. Requires GITHUB_TOKEN in .env.", "parameters": {"properties": {"pr_number": {"description": "The PR number to torture test", "type": "integer"}}, "type": "object"}}
...
def run(args: dict) -> dict:
    ...
    if py_files:
        ...
        for f in py_files:
            ...
            try:
                ...
                r = subprocess.run([venv_python, "-m", "py_compile", tmp_path],
    ...
    if js_files:
        ...
        for f in js_files:
            ...
            try:
                raw_resp = requests.get(raw_url, headers={"Authorization": f"token {token}"}, timeout=15)
                if raw_resp.status_code != 200:
                    js_results.append({"file": f["filename"], "status": "skip", "message": f"Download failed: {raw_resp.status_code}"})
                    continue
                ...
                r = subprocess.run(["node", "--check", tmp_path], capture_output=True, text=True, timeout=15)
                os.unlink(tmp_path)
        ...
        tests.append({"test": "js_syntax_check", "status": "skip", "message": "No .js files in PR"})
    ...
    if gitignore_file:
        ...
        tampering = []
        sensitive_paths = ['.env', 'sessions/', 'identity/', 'vaultbot_index/', 'trash/',
                          'vaultbot_venv/', 'Memory/', 'Knowledge/', 'learningMaterial/',
        ...
        for line in patch.split('\n'):
            if line.startswith('-') and not line.startswith('---'):
                ...
                for sp in sensitive_paths:
                    if sp in removed:
                        tampering.append(f"Removed ignore rule for '{removed}' (matches sensitive path '{sp}')")
        ...
        tests.append({"test": "gitignore_tampering", "status": "skip", "message": ".gitignore not modified"})
    ...
    MALWARE_PATTERNS = [
        ...
        (r"subprocess\..*shell\s*=\s*True.*\binput\b", "subprocess with user input + shell=True"),
        ...
        (r"https?://(?!github\.com|api\.github\.com|raw\.githubusercontent\.com)", "Network call to non-GitHub host"),
    ]
    ...
    ALLOWED_PREFIXES = [
        "vaultbot_stuff/", ".obsidian/plugins/vaultbot/", ".gitignore",
        "LICENSE", "README", "CONTRIBUTING", "SECURITY", "setup.ps1", "setup.sh",
        ".pre-commit", ".github/",
    ]
    path_violations = []
    for f in pr_files:
        fname = f["filename"]
        if not any(fname.startswith(p) or fname == p.rstrip("/") for p in ALLOWED_PREFIXES):
