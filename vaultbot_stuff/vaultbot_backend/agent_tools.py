...
def build_system_prompt(vault_context: str, autonomous_state: dict[str, Any],
    ...
    return (
        ...
        f"   CONSOLIDATION: when you mark a step completed, the framework "
        f"replaces the raw tool calls + results + thinking for that step "
        f"with a compact summary (what was accomplished + lessons learned + "
        f"key facts the next step needs). You see the summaries, not the "
        f"raw chatter. Plan your steps so each step's summary carries the "
        f"facts the next step needs — the raw output won't be there next "
        f"round.\n"
        f"   FINAL SYNTHESIS: when the last step is done, the framework "
        f"asks you for ONE closing summary. Write it as a complete, self-"
        f"contained answer — the user saw only the step headers, not the "
        f"details. This closing summary is what the user actually reads.\n"
        ...
        f"task.\n"
        f"   TURN PROTOCOL / PHASE STATE MACHINE: The framework enforces a "
        f"strict plan -> act -> synthesize cycle. "
        f"PLAN phase: before a plan exists, call plan_task with a goal and steps. "
        f"No other tools, no prose answer. "
        f"ACT phase: a plan exists. Start each work step with update_task(in_progress), "
        f"do the work with the tools you need, then mark it completed. "
        f"SYNTHESIZE phase: when every step is done, write the final answer as prose. "
        f"No tool calls. The framework reads the structural signal (tool_calls vs. text) "
        f"and enforces the phase. Do not narrate intent and stop - either call the "
        f"correct tool for the phase or write the final answer.\n"
...
def build_system_prompt_briefing(autonomous_state: dict[str, Any],
    ...
    return (
        ...
        f"key facts the next step needs). You see the summaries, not the "
        f"raw chatter. Plan your steps so each step's summary carries the "
        f"facts the next step needs.\n"
        f"   FINAL SYNTHESIS: when the last step is done, the framework asks "
        f"you for ONE closing summary. Write it as a complete, self-"
        f"contained answer — the user saw only the step headers, not the "
        f"details. This closing summary is what the user actually reads.\n"
        f"   READING CODE: When using code_read, use start_line/end_line to "
        f"read specific sections. If the file is large, read it in chunks "
        ...
        f"No tool calls. The framework reads the structural signal (tool_calls vs. text) "
        f"and enforces the phase. Do not narrate intent and stop - either call the "
        f"correct tool for the phase or write the full answer.\n"
