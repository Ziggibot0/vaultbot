...
async def _run_procedure_direct(
    ...
    )  # vaultbot_backend -> vaultbot_stuff -> vault root
    ...
    try:
                    ...
                    if isinstance(_preflight_chain, list) and _preflight_chain:
                        ...
                        for _chain_proc in _preflight_chain:
                            ...
                            try:
                                ...
                                _chain_cartridge = (
                                    str(_fm.get("model_cartridge", "big"))
                                    .strip()
                                    .lower()
                                    or "big"
                                )
        ...
        if _conv_results:
            try:
                ...
                session_logger.log(
                    "conversation_context_inject_failed", {"error": str(e)}
                )
        ...
        if _preflight_chain:
            ...
            session_logger.log(
                "preflight_chain_injected",
                {
                    ...
                    "chain": _preflight_chain,
                    "executed": sum(
                        1 for p in _preflight_results if not p.get("pending")
                    ),
            ...
            try:
                ...
                if _trivial_client is not None:
                    ...
                    if len(_trivial_text) >= 10:
                        ...
                        await svc.manager.send_personal_message(
                            json.dumps(
                                {"type": "answer_chunk", "content": _trivial_text}
                            ),
                            websocket,
                            session_logger=session_logger,
                        )
                        await svc.manager.send_personal_message(
                            json.dumps(
                                {"type": "answer_done", "content": _trivial_text}
                            ),
                            websocket,
                            session_logger=session_logger,
                        )
            ...
            try:
                ...
                session_logger.log(
                    "drift_feedback",
                    {
                        ...
                        "helpful": (
                            len(final_answer) > 50 and not first_round_researched
                        ),
...
async def execute_agent_tool(
    ...
    if tool_name == "execute_procedure":
        ...
        try:
            ...
            session_logger.log(
                "procedure_lookup_failed", {"proc": proc_name, "error": str(e)}
            )
        ...
        try:
            ...
            if not _allowed:
                session_logger.log(
                    "procedure_blocked", {"procedure": proc_name, "status": _status}
                )
                return {
                    "error": f"procedure blocked: {proc_name}",
        ...
        try:
            ...
            session_logger.log("tracker_log_path_forward_failed", {"error": str(e)})
