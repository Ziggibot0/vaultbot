...
proc_path = Path(vault_path) / "vaultbot_stuff" / "System" / "Procedures" / f"{target}.md"
current_content = ""
if proc_path.exists():
...
- Return ONLY the markdown, no commentary"""
    ...
    if patched_content and len(patched_content) > 50:
        write_result = vault_safe_write(
            f"vaultbot_stuff/System/Procedures/{target}.md",
            patched_content
        )
...
step1 = json.loads(get_prior(prior_results, 1))
target = step1.get("procedure_name", "")
file_path = f"vaultbot_stuff/System/Procedures/{target}.md"
