...
result = json.dumps({
    ...
    "issues": issues,
    "verification": verification,
    "action": "Report issues to the operator if any. Don't silently pass a note with verification failures.",
})
```
...
- [[Calibration-via-Operator-Feedback]] — using the operator's corrections to calibrate
