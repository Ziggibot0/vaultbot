...
falsifiable_if: "retrieval is rated as high-quality by this procedure but the operator reports that key notes were missing from the retrieved subgraph"
applies_to:
  - retrieval
  - evaluation
  - rag
...
# Grow this set organically by adding queries where the operator reported misses.
test_queries = [
...
Compare current metrics to previous runs. If recall or precision dropped, investigate what changed. Report findings to the operator.
...
If retrieval is rated as high-quality by these metrics but the operator consistently reports missing notes, the metrics are wrong, not the operator. Log it as a procedure failure.

## Related

- [[RAG-Evaluation-for-FUSED-Retrieval]] — the architecture this procedure implements
- [[Calibration-via-Operator-Feedback]] — the operator's corrections grow the test set
