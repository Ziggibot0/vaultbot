...
falsifiable_if: "an audit following this procedure passes a category that the operator identifies as broken, or fails a category that the operator identifies as working"
applies_to:
  - self-evaluation
  - vault-health
  - system-reliability
...
Run this procedure periodically (monthly) or when the operator asks "how are you doing?" or "audit yourself." This is structured self-critique — not a victory lap. The goal is to find real problems and fix them.
...
4. Empathy with the creator — evaluate against the operator's goals, not abstract ML benchmarks.
...
result = json.dumps({
    ...
    "action": "Call vaultbot_status(). Synthesize all findings into a prioritized report. Report to the operator bottom-line-up-front. Fix Critical issues before reporting done.",
})
```
...
- [[Calibration-via-Operator-Feedback]] — using the operator's corrections
