...
the operator's directive makes the constraint clear: **automate as much as possible so the LLM has very little to do.** The framework and vault should handle pattern extraction. The LLM should only synthesize pre-extracted patterns into prose.
...
**How to adapt for VaultBot**: Compare sessions where things went well (the operator said "yes", "go ahead", "cool beans") vs. sessions where things went poorly (the operator corrected, said "no", said "I thought you already did that"). The differences between these trajectories ARE the rules of thumb. This comparison can be done deterministically by scanning chat logs for the operator's response patterns.

## The Deterministic Consolidation Pipeline

### Design Principle: Extract Mechanically, Synthesize with LLM

The key constraint from the operator: **the framework does most things, the LLM does very little.** This means:
...
- the operator's response (positive/negative/neutral — detectable from keywords)
...
| **the operator's preferences** | Scan for correction patterns + positive responses | "the operator prefers X, dislikes Y" |
...
- If validation fails, the note is flagged for human review (the operator) or re-synthesis
...
4. **the operator's corrections compound.** Each correction becomes episodic evidence that feeds the next consolidation cycle. Over time, the semantic layer encodes "what the operator has taught me" as reusable knowledge, not just scattered chat log entries.
...
- `extract_preference_signals(experiences)` → the operator's positive/negative response patterns
