...
async def run_background_tasks(
    ...
    # actually supported by its source note. This is the idle-time
    # "I don't worry about hallucinations" guarantee — it runs in the
    # background (NOT on the chat critical path) and upgrades the trust
    # badge from "grounded" to "verified" once the per-claim checks
    # complete. Only fires when the answer actually cites vault notes
    # (a greeting or "I don't know" has nothing to verify).
    async def _run_provenance_verify_bg():
        try:
            import re as _re
            ...
            if not _links:
                return  # nothing cited — nothing to verify
            # IDK escape hatch: if the answer is an admission of ignorance
            # ("I don't know — nothing in the vault covers this"), there
            # are no factual claims to verify. The wikilinks in an IDK
            # answer are just the irrelevant notes the model was told it
            # could cite — verifying their entailment is tautological and
            # wastes minutes of small-model time.
            from citation_gate import detect_idk

            if detect_idk(final_answer or ""):
                session_logger.log("provenance_verify_skipped_idk", {})
                return
            from chat_preflight import run_procedure_direct

            _verify = await run_procedure_direct(
                svc,
                "Verify-Answer-Entailment",
                proc_args={"answer": final_answer},
                session_logger=session_logger,
                user_message=user_message,
                websocket=websocket,
            )
            if _verify.get("error"):
                session_logger.log(
                    "provenance_verify_failed", {"error": _verify.get("error")}
                )
                return
