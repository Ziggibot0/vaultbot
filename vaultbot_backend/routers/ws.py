...
# agent proactively resumes work without waiting for Sean to send a message.
...
async def websocket_endpoint(websocket: WebSocket,
    ...
    Sean and spawns handle_chat with a synthetic "continue" trigger. This
    means after a restart, the agent wakes up and continues working without
    Sean having to send a wake-up message. See [[Auto-Resume-Directive]].
    """
    ...
    if _RESTART_CONTEXT_PATH.exists():
        ...
        # with the receive loop below, so Sean can still interrupt with
        ...
        async def _auto_resume():
            try:
                await handle_chat(
                    ...
                    "Don't ask Sean to re-explain anything. Just do it.",
                    session_logger)
