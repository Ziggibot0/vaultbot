...
def build_system_prompt_briefing(
    ...
    return (
        ...
        f"- DEFAULT TO PROCEDURES, BE PICKY WITH TOOLS. A procedure is a "
        f"macro over tools you already have (Python + [llm:] steps) — it adds "
        f"zero new surface area. Before tool_create, ask: can I express this "
        f"as a procedure over existing tools? If yes, write the procedure. "
        f"Only build a tool when it's a genuinely NEW primitive used across "
        f"many unrelated tasks, not a composition. Every tool's schema taxes "
        f"your context window on EVERY turn even when unused; a procedure "
        f"only loads when retrieval surfaces it. And building a tool teaches "
        f"you to build tools — don't train yourself away from procedures.\n"
