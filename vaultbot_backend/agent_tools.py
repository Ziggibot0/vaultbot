...
def build_system_prompt(vault_context: str, autonomous_state: dict[str, Any],
    ...
    return (
        ...
        f"plan, and it will STOP THE LOOP for you if you spend too many "
        f"rounds only reading the vault without producing an answer or "
        f"checking off steps. So plan early, then work the plan. After each "
        f"tool round, call update_task to mark the step in_progress → "
        f"completed. When ALL steps are completed, the loop ends "
        f"automatically — synthesize your final answer. Do NOT keep calling "
        f"tools after the plan is done. This is how you stay on track "
