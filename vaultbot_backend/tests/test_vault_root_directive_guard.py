...
belong under vaultbot-stuff/System/Identity/. The `_is_root_directive` helper is
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Nested under vaultbot-stuff/System/Identity/ — allowed.
        ("vaultbot-stuff/System/Identity/Autonomy-Directive.md", False),
        ("vaultbot-stuff/System/Identity/IDK-Fallback-Directive.md", False),
        ("vaultbot-stuff/System/Identity/Sean-Communication-Preferences.md", False),
        # Normal user notes — allowed.
        ("User/My-Note.md", False),
        ("User/Research-Roadmap.md", False),
        ("vaultbot-stuff/Knowledge/Research/My-Note.md", False),
...
def test_run_allows_nested_directive(tmp_path, monkeypatch):
    """A directive under vaultbot-stuff/System/Identity/ is allowed."""
    monkeypatch.setattr(vault_safe_write, "VAULT_ROOT", tmp_path)
    target = tmp_path / "vaultbot" / "System" / "Identity"
    target.mkdir(parents=True, exist_ok=True)
    result = vault_safe_write.run(
        {
            "file_path": "vaultbot-stuff/System/Identity/Autonomy-Directive.md",
            "content": "# Autonomy\n",
        }
    )
...
def test_vault_append_allows_nested_directive(tmp_path, monkeypatch):
    """vault_append allows a directive under vaultbot-stuff/System/Identity/."""
    monkeypatch.setattr(vault_append, "VAULT_ROOT", tmp_path)
    target = tmp_path / "vaultbot" / "System" / "Identity"
    target.mkdir(parents=True, exist_ok=True)
    result = vault_append.run(
