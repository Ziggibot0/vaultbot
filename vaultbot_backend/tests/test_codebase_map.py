...
def _point_at_tmp(tmp_path, monkeypatch):
    """Point paths.VAULT_ROOT at tmp_path so run() resolves its map there.

    run() resolves the map via paths.VAULT_ROOT (the user's vault root).
    Setting VAULT_PATH to tmp_path makes the map resolve to
    tmp_path/vaultbot-stuff/Knowledge/Architecture/Codebase-Map.md.
    """
