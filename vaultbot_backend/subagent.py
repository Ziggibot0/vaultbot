...
def build_subagent_code(topic: str, depth: str = "deep") -> str:
    ...
    code = (
        ...
        "    else:\n"
        "        _log('[subagent] LLM client unavailable - using extractive fallback')\n"
        "\n"
        ...
        "    else:\n"
        "        # Extractive fallback: overwrite with richer markdown\n"
        "        from vault_guard import assert_writable\n"
        "        assert_writable(Path(note_path))\n"
        "        md = engine.synthesize_note_markdown(report, summary)\n"
        "        Path(note_path).write_text(md, encoding='utf-8')\n"
        "\n"
        "        # Try LLM-assisted restructuring (one call, optional)\n"
        "        if _oc is not None:\n"
        ...
        "                else:\n"
        "                    _log('[subagent] structured note below floor, kept extractive')\n"
        "            except Exception as _se:\n"
        "                _log(f'[subagent] structured synthesis failed: {_se}')\n"
        "\n"
...
def run_research_subagent(topic: str, depth: str = "deep",
    ...
    try:
        ...
        if session_logger and stderr:
            session_logger.log("subagent_stderr", {"tail": stderr[-500:]})

        if not stdout:
            if session_logger:
                session_logger.log("subagent_no_output", {
                    "returncode": result.returncode,
                    "stderr_tail": stderr[-300:] if stderr else "",
                })
            return {"status": "error",
                    "error": f"subagent produced no output (rc={result.returncode})",
                    "subagent": True}
        ...
        try:
            ...
            for line in stdout.split("\n"):
                ...
                if session_logger:
                    session_logger.log("subagent_json_parse_failed", {
        ...
        if session_logger:
            session_logger.log("subagent_complete", {
        ...
        if session_logger:
            session_logger.log("subagent_timeout", {"topic": topic[:80]})
        return {"status": "error",
