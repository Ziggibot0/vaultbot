...
def _build_research_wrapper(topic: str, depth: str) -> str:
    ...
    return (
        ...
        "    # The extractive markdown above is a flat bullet dump. Try to\n"
        "    # restructure it into a proper research note (frontmatter, H2\n"
        "    # sections, wikilinks) via one LLM call. Falls back to the\n"
        "    # extractive markdown if the LLM is unavailable or produces a\n"
        "    # note below the safety floor — never worse than current.\n"
        "    try:\n"
        "        from ollama_client import OllamaClient\n"
        "        from vault_graph import VaultGraph\n"
        "        _oc = OllamaClient()\n"
        "        _graph = VaultGraph(vault_path=vault_path)\n"
        "        _titles = list(_graph.nodes.keys()) if _graph.nodes else []\n"
        ...
        "            _log('[subagent] structured note written')\n"
        "        else:\n"
        "            _log('[subagent] structured note below floor, kept extractive')\n"
        "    except Exception as _e:\n"
        "        _log(f'[subagent] LLM structuring skipped: {_e}')\n"
        "\n"
