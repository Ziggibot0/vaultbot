...
# Determine vault root from this file's location
# (custom_tools/vault_safe_write.py -> parent.parent = vaultbot_backend/ -> parent = vault root)
try:
...
def _is_sacred_journal(file_path: Path) -> bool:
    """Check if the filename is a date-only filename (Sean's personal journal)."""
    stem = file_path.stem
