...
def run(args: dict) -> dict:
    ...
    if not vault_path:
        # Auto-detect via central paths.py (vault is a sibling of the backend).
        ...
        cluster_profiles.append(
            {
                ...
                "size": len(comm),
                "top_dirs": [
                    {"dir": d, "count": c, "pct": round(100 * c / len(comm))}
                    for d, c in top_dirs
                ],
    ...
    cross_summary = []
    for (ca, cb), edge_list in sorted(
        cross_edges.items(), key=lambda x: len(x[1]), reverse=True
    ):
        cross_summary.append(
            {
                ...
                "edges": [
                    {"from": node_to_path.get(a, a), "to": node_to_path.get(b, b)}
