...
IGNORED_DIRS = {"vaultbot_venv", "vaultbot_index", "sessions", "partials",
                 ".git", ".obsidian", "vaultbot_backend", "learningMaterial",
...
def run(args: dict) -> dict:
    ...
    if not vault_path:
        # Auto-detect: parent of vaultbot_backend
        backend_path = Path(__file__).resolve().parent.parent
        if (backend_path / "vaultbot_backend").exists():
            vault_path = str(backend_path)
        else:
            vault_path = str(backend_path.parent)
