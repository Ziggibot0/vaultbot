...
def existing_note_titles(svc: Services) -> dict:
    ...
    try:
        for _name, node in (svc.vault_graph.nodes or {}).items():
            ...
            if "vaultbot-stuff/Knowledge/Textbooks" + os.sep not in fp + os.sep:
...
def link_outbound(note_path: str, title_map: dict) -> int:
    ...
    try:
        ...
        for stem_lower, _fp in title_map.items():
            ...
            pattern = re.compile(
                r"(?<!\[\[)\b" + re.escape(stem_lower) + r"\b(?!\]\])", re.IGNORECASE
            )
...
def cross_link_textbooks(
    svc: Services,
    new_abs_paths: list[str],
    emb_by_path: dict,
    source_keys: set | None = None,
) -> dict:
    ...
    try:
        textbooks_dir = (
            Path(os.getenv("VAULT_PATH", ".")) / "vaultbot-stuff/Knowledge/Textbooks"
        )
        ...
        for new_path in new_abs_paths:
            try:
                ...
                hits = svc.vault_indexer.search_by_vector(
                    _np.asarray(emb, dtype=_np.float32), k=15
                ...
                for h in hits:
                    ...
                    if (
                        "vaultbot-stuff/Knowledge/Textbooks" + os.sep
...
def insert_related_block(note_path: str, links: list) -> int:
...
async def weave_textbook_notes(
    svc: Services,
    ingest_result: dict,
    ...
    out: dict[str, Any] = {
        "indexed": 0,
        "outbound_links_added": 0,
        "amem_evolved": 0,
        "amem_links_added": 0,
        "cross_links_added": 0,
        "notes_cross_linked": 0,
        "notes": [],
        "status": "complete",
    }
    ...
    try:
        ...
        for idx, (rel, abs_path) in enumerate(zip(note_rels, abs_paths, strict=False)):
            ...
            added = await loop.run_in_executor(None, link_outbound, abs_path, title_map)
            ...
            ev = await loop.run_in_executor(
                None,
                lambda c=content, a=abs_path: svc.amem.evolve_on_create(
                    a,
                    c,
                    heuristic_only=True,
        ...
        if card_paths:
            ...
            try:
                ...
                textbooks_dir = (
                    ...
                    / "vaultbot-stuff/Knowledge/Textbooks"
                )
        ...
        if websocket is not None:
            await _send_progress(
                ...
                "weaving_done",
                {
                    ...
                    "message": (
                        f"Done: {out['outbound_links_added']} outbound links, "
