...
**What it does:**
- Detects when Sean corrects an output (keyword matching: "no", "wrong", "actually", "that's not", "fix")
- Logs the correction with: what was said, which quality gate passed/failed, which notes were in context
- Computes calibration metrics over time: precision/recall of vault_lint, safe_write, etc. vs Sean's ground truth
...
- Uses Sean's corrections as ground truth (when he says "you missed X", that note should have been retrieved)
...
**Risk:** Moderate. Uses LLM (cost + latency), and entailment checking is inherently fuzzy. Mitigation: log everything, let the procedure tracker track success rate, and use Sean's corrections to calibrate. The verifier is advisory, not blocking — it flags issues but doesn't prevent note creation.
...
- Manual: Sean reviews and confirms "yes, this is what good looks like"
...
**Mitigation:** Truncation preserves frontmatter + summary + first section. Drop log records what was cut. Sean can review the log and adjust priorities.
...
**Mitigation:** Start with logging only (no metrics). Ground truth accumulates from Sean's corrections over time. The evaluator is useful even without metrics — it tracks what was retrieved for each query, which is valuable for debugging.
...
### Risk 5: Exemplars don't match what Sean wants
**Mitigation:** Sean reviews each exemplar. They're just notes — easy to delete or revise. The exemplar design principles are already in [[Exemplar-Note-Design]] and can be refined.
...
4. **Sean review** — he confirms the module does what he expects
...
- [[How-to-Evaluate-Retrieval-Quality]] — procedural note for Phase 3
- [[How-to-Verify-Claims-in-a-Research-Note]] — procedural note for Phase 4
