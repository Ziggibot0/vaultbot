...
class VaultBotPlugin extends Plugin {
	...
	_ensureIgnoredDirs() {
		...
		// The backend + .venv now live OUTSIDE the vault (siblings of vault/),
		// so Obsidian's file watcher never sees them — no ignore filters are
		// needed for the engine. The transparent vaultbot-stuff/ folder stays
		// visible on purpose (that's the transparency layer).
		const required = [];
	...
	onceBackendReady(timeoutMs = 30000, intervalMs = 500) {
		...
		this._backendReadyPromise = (async () => {
			...
			const pidFile = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'vaultbot.pid');
			const fs = require('fs');
	...
	_venvPythonExe(vaultRoot) {
		...
		const candidates = process.platform === 'win32'
			? [path.join(path.dirname(vaultRoot), '.venv', bin, 'pythonw.exe'),
			   path.join(path.dirname(vaultRoot), '.venv', bin, 'python.exe')]
			: [path.join(path.dirname(vaultRoot), '.venv', bin, 'python')];
		const fs = require('fs');
	...
	async startMcpServerIfNeeded() {
		...
		try {
			...
			const mcpPy = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'mcp_server.py');
			const fs = require('fs');
	...
	_getAuthToken() {
		try {
			...
			const tokenPath = path.join(path.dirname(vaultRoot), 'vaultbot_backend', '.vaultbot_auth_token');
	...
	async performSelfUpdate(onProgress, ref) {
		...
		const backendDir = path.join(path.dirname(vaultRoot), 'vaultbot_backend');
		...
		try {
			...
			if (fs.existsSync(gitDir)) {
				...
				try {
					...
					const reqPath = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'requirements.txt');
	...
	_readBackendLog(vaultRoot, maxLines = 50) {
		const fs = require('fs');
		const logFile = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'backend.log');
		try {
	...
	async startBackendIfNeeded() {
		...
		try {
			...
			const mainPy = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'main.py');
			const logFile = path.join(path.dirname(vaultRoot), 'vaultbot_backend', 'backend.log');
...
// Open the log file in append mode. The running backend inherits the
		...
		try {
			...
			const altLog = path.join(path.dirname(vaultRoot), 'vaultbot_backend', `backend-${stamp}.log`);
			try {
