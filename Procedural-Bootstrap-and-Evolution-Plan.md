...
| [[Sean-Communication-Preferences]] | "Keep it short, lead with outcome" | Already deterministic: bullet points, bottom line up front |
...
|---|---|
| How to structure a research note | Zettelkasten guides, academic writing templates |
| How to evaluate source credibility | Research methodology guides, media literacy frameworks |
| How to decide when to research vs answer | Agent framework decision trees, RAG architecture docs |
...
| How to validate LLM output | Schema validation patterns, QA checklists |
...
**Sean's role here:** Sean's corrections are the highest-weight quality signal. If Sean says "this is wrong," the procedure is immediately flagged for re-research, regardless of the failure count. Sean doesn't need to write procedures -- he just needs to correct bad output. The system figures out why the output was bad and finds a better procedure.
...
- Sean's corrections are ground truth. If Sean says "this is wrong," that overrides everything.

**Residual risk:** A bad procedure that produces output that *passes validation but is actually wrong.* This is the "looks correct but isn't" problem. The only defense is Sean's judgment. The system should surface low-confidence procedures for Sean's review.
...
- Sean's corrections are the calibration signal. If Sean says "this should have passed," the validation is too strict. If Sean says "this should have failed," it's too loose.
...
- [[Sean-Communication-Preferences]] -- how Sean wants to be communicated with
...
2. **The validation layer is not optional** — it's the only thing standing between the 30B's ~66-86% success rate and Sean's vault. Without it, bad output silently accumulates.
...
| Human rater | Sean's corrections | Strongest — ground truth |

**What this means for the plan**: The plan is using the RIGHT verification level for each task. Code changes get safe_write (strongest). Note quality gets vault_lint (medium). Everything gets Sean's corrections (ground truth). The plan should NOT add an LLM judge — the research shows it doesn't work reliably.
...
This is already handled by the existing README. The plan doesn't change this. But the plan should acknowledge it so Sean knows the full picture.
...
**Mitigation**: The validation hierarchy is already correct (see Finding 6). Code changes get the strongest verification (safe_write). Note quality gets medium verification (vault_lint). Sean's corrections are ground truth. The plan should NOT add an LLM judge — the research shows it doesn't work.

**Residual risk**: vault_lint catches structural issues (broken wikilinks, missing frontmatter) but not semantic ones (wrong reasoning, bad arguments). The only defense against semantic errors is Sean's corrections. The system should surface low-confidence outputs for Sean's review.
...
2. **The 30B's ability to follow procedures reliably.** The research shows 93-96% per-call reliability, but compounding means 66-86% for multi-step procedures. The validation layer catches the failures, but the system will produce failed output ~14-34% of the time on multi-step tasks. This is acceptable for a research assistant (Sean can correct it), but it's not "set and forget."
...
2. **Whether the autonomous researcher will correctly identify procedural gaps.** The failure log tracks failures by task type, but mapping "a validation failure on note X" to "we need a procedure for task type Y" requires some inference. This might need human (Sean) input to calibrate.
...
|---|---|---|---|
| [[How-to-Structure-a-Research-Note]] | 5 (Zettelkasten guides) | 8, 0 broken | ✅ Pass |
