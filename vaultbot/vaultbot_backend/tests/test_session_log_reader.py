...
def _evt(event: str, data: dict | None = None, ts: float = 1000.0, sid: str = "test-uuid-1234") -> dict:
...
def test_parse_canonical_conversation(tmp_path: Path):
    """Fix #3: chat_begin and assistant_response are the canonical conversation events."""
    f = tmp_path / "test-uuid-1234.jsonl"
    # session_start writes title at the TOP LEVEL, not inside data
    _write_session(f, [
        {"event": "session_start", "session_id": "test-uuid-1234", "timestamp": 1000.0, "started_at": "2026-08-20T10:00:00+00:00", "title": "Test"},
...
def test_parse_assistant_response_text_fallback(tmp_path: Path):
    """Some assistant_response events use 'text' instead of 'content' (chat_turn_prep.py:1243)."""
    f = tmp_path / "test.jsonl"
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
...
def test_parse_token_usage(tmp_path: Path):
    ...
    _write_session(f, [
        ...
        _evt("session_token_total", {"prompt_tokens": 300, "completion_tokens": 130, "total_tokens": 430}),
    ])
    summary = parse_session_log(f)
...
def test_parse_websocket_fallback(tmp_path: Path):
    """Fix #3: legacy sessions without chat_begin fall back to websocket_message parsing."""
    f = tmp_path / "test.jsonl"
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "Legacy"}),
        _evt("websocket_message", {"direction": "in", "payload": {"type": "chat", "message": "old style question"}}, ts=1000.0),
        _evt("websocket_message", {"direction": "out", "payload": {"type": "answer_done", "content": "old style answer"}}, ts=1001.0),
    ])
    summary = parse_session_log(f)
    assert len(summary["turns"]) == 2
    assert summary["turns"][0]["role"] == "user"
...
def test_parse_websocket_answer_chunks_assembled(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("websocket_message", {"direction": "in", "payload": {"type": "chat", "message": "hi"}}, ts=1000.0),
        _evt("websocket_message", {"direction": "out", "payload": {"type": "answer_chunk", "content": "Hello "}}, ts=1001.0),
        _evt("websocket_message", {"direction": "out", "payload": {"type": "answer_chunk", "content": "world!"}}, ts=1002.0),
        _evt("websocket_message", {"direction": "out", "payload": {"type": "answer_done", "content": ""}}, ts=1003.0),
    ])
    summary = parse_session_log(f)
    assert len(summary["turns"]) == 2
...
def test_parse_tool_call_with_call_id(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("tool_call_requested", {"call_id": 1, "tool": "vault_search", "args": {"query": "test"}, "round": 0}, ts=1000.0),
        _evt("tool_call_result", {"call_id": 1, "tool": "vault_search", "round": 0, "duration_ms": 42.5, "result_keys": ["results"]}, ts=1001.0),
        _evt("tool_call_requested", {"call_id": 2, "tool": "vault_read_note", "args": {"title": "X"}, "round": 1}, ts=1002.0),
        _evt("tool_call_result", {"call_id": 2, "tool": "vault_read_note", "round": 1, "duration_ms": 10.0, "result_keys": ["content"]}, ts=1003.0),
    ])
    summary = parse_session_log(f)
...
def test_parse_tool_call_legacy_fallback(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("tool_call_requested", {"tool": "vault_search", "args": {"query": "test"}, "round": 0}, ts=1000.0),
        _evt("tool_call_result", {"tool": "vault_search", "round": 0, "duration_ms": 15.0, "result_keys": ["results"]}, ts=1001.0),
    ])
    summary = parse_session_log(f)
...
def test_parse_log_tool_call_combined_event(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("tool_call", {"call_id": 1, "tool": "free_search", "method": "search", "inputs": {"q": "test"}, "outputs": {"hits": 3}, "duration_ms": 100.0}, ts=1000.0),
    ])
    summary = parse_session_log(f)
    assert len(summary["tool_calls"]) == 1
    tc = summary["tool_calls"][0]
...
def test_parse_exceptions(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("exception", {"error": "ValueError: bad thing", "context": "tool_vault_search", "traceback": "..."}),
...
def test_find_session_by_title_substring(tmp_path: Path):
    ...
    _write_session(f, [
        {"event": "session_start", "session_id": "ccc33333-3333-3333-3333-333333333333", "timestamp": 1000.0, "started_at": "x", "title": "New Session"},
        {"event": "session_title", "session_id": "ccc33333-3333-3333-3333-333333333333", "timestamp": 1001.0, "title": "Temporal Awareness Debug"},
    ])
...
def test_format_transcript_conversation_filter(tmp_path: Path):
    ...
    _write_session(f, [
        ...
        _evt("tool_call", {"call_id": 1, "tool": "vault_search", "method": "search", "inputs": {}, "outputs": {}}),
    ])
    summary = parse_session_log(f)
...
def test_format_transcript_tools_filter(tmp_path: Path):
    ...
    _write_session(f, [
        _evt("session_start", {"started_at": "2026-01-01T00:00:00+00:00", "title": "T"}),
        _evt("chat_begin", {"user_message": "hello"}),
        _evt("tool_call", {"call_id": 1, "tool": "vault_search", "method": "search", "inputs": {"q": "x"}, "outputs": {}, "duration_ms": 50.0}),
    ])
    summary = parse_session_log(f)
...
def test_format_transcript_all_filter(tmp_path: Path):
    ...
    _write_session(f, [
        ...
        _evt("tool_call", {"call_id": 1, "tool": "x", "method": "m", "inputs": {}, "outputs": {}}),
