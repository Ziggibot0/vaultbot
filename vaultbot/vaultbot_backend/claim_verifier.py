...
class ClaimVerifier:
    ...
    def __init__(
        ...
        self.log_path = log_path or os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "claim_verification_log.json"
        )
    ...
    def _parse_sources_section(self, note_content):
        ...
        for line in sources_text.split("\n"):
            ...
            archived_match = re.search(
                r"\[\[learningMaterial/web/([^|]+?)(?:\|archived)?\]\]", line
            )
            archived_filename = (
                archived_match.group(1).strip() if archived_match else None
            )
            sources[title.lower()] = {
                "title": title,
                "url": url,
    ...
    def _llm_extract_claims(self, synthesis_text):
        prompt = (
            "You are a claim extraction system. Extract all atomic factual claims "
        ...
        try:
            ...
            response = _verify_client.chat(
                [{"role": "user", "content": prompt}], temperature=0.1, stream=False
            )
            raw = ""
            if isinstance(response, dict):
                raw = response.get("message", {}).get("content", "") or response.get(
                    "response", ""
                )
    ...
    def extract_claims(self, note_content):
        synthesis_match = re.search(
            r"##\s*(?:Key Findings|Synthesis|Summary)\s*\n(.*?)(?:\n##\s|\Z)",
            note_content,
            re.DOTALL,
        )
    ...
    def _llm_check_entailment(self, claim, source_text):
        ...
        prompt = (
            "You are a fact-checking system. Given a source text and a claim, "
    ...
    def _deterministic_check_entailment(self, claim, source_text):
        ...
        if not key_phrases:
            ...
            return {
                "verdict": self.UNSUPPORTED,
                "reasoning": "Insufficient word overlap",
            }
        ...
        if match_ratio >= 0.3:
            return {
                ...
                "reasoning": f"{match_ratio:.0%} of key phrases found in source",
            }
        elif match_ratio >= 0.1:
            return {
                "verdict": self.UNSUPPORTED,
                "reasoning": f"Only {match_ratio:.0%} of key phrases found",
            }
        else:
            return {
                "verdict": self.UNSUPPORTED,
                "reasoning": "No key phrase overlap with source",
            }
    ...
    def verify_note(self, note_path):
        ...
        for claim_data in claims:
            ...
            source_title = claim_data.get("source") or self._extract_source_citation(
                claim_text
            )
            if not source_title:
                result = {
                    "claim": claim_text,
                    "source": None,
                    "verdict": self.UNSOURCED,
                    "reasoning": "No source citation found",
                }
            ...
            if not source_info:
                result = {
                    "claim": claim_text,
                    "source": source_title,
                    "verdict": self.SOURCE_NOT_FOUND,
                    "reasoning": "Source not found in note sources",
                }
            ...
            if not source_text:
                result = {
                    ...
                    "source": source_title,
                    "verdict": self.SOURCE_NOT_FOUND,
                    "reasoning": "Could not load archived source file",
                }
                stats[self.SOURCE_NOT_FOUND] += 1
                verified_claims.append(result)
                continue
            clean_claim = re.sub(
                r"\s*\[sources?:\s*[^\]]+\]", "", claim_text, flags=re.IGNORECASE
            )
        ...
        report = {
            ...
            "verified": stats[self.SUPPORTED],
            "unsupported": stats[self.UNSUPPORTED],
            "contradicted": stats[self.CONTRADICTED],
            "unsourced": stats[self.UNSOURCED],
            "source_not_found": stats[self.SOURCE_NOT_FOUND],
    ...
    def _log_verification(self, report):
        ...
        compact = {
            ...
            "unsourced": report["unsourced"],
            "source_not_found": report["source_not_found"],
            "failed_claims": [
                {
                    "claim": c["claim"][:200],
                    "verdict": c["verdict"],
    ...
    def get_verification_summary(self):
        ...
        return {
            ...
            "failure_rate": (total_unsupported + total_contradicted) / total_claims
            if total_claims
            else 0,
        }
        ...
        for log in logs:
                ...
                gaps.append(
                    {
                        ...
                        "issue": f"{contradicted} contradicted claim(s)",
                        "severity": "high",
            ...
            if unsourced > 0:
                gaps.append(
                    {
                        "note_path": log["note_path"],
                        "issue": f"{unsourced} unsourced claim(s)",
                        "severity": "low",
                        "failed_claims": log.get("failed_claims", []),
                    }
                )
