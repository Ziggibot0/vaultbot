...
Four channels are fused:
  ...
  c. Graph channel    -- wikilink neighbors of vector hits, score = 0.5 x vector score
  d. Backlink channel -- backlinks of vector hits, score = 0.7 x vector score
...
multi-channel agreement (x1.3) and hub-ness (x1.1), then truncated to top-k.
...
class FusedRetriever:
    ...
    # vector channel's top hit).  The rerank boosts (x1.3 all-channel, x1.1
    ...
    def _vector_channel(
        ...
        over-fetches DRIFT_OVERFETCH x k candidates, reconstructs each one's
    ...
    def _drift_rerank(
        ...
        # drift-adjusted distance `norm(drifted_emb - query_emb)` is ~20x the
    ...
    def _graph_channel(
        ...
        """1-hop wikilink neighbors of vector hits. Score = GRAPH_BOOST x vector score.
    ...
    def _backlink_channel(
        ...
        """Backlinks of vector hits. Score = BACKLINK_BOOST x vector score.
    ...
    def _merge(
        ...
        # fold in graph + backlink + lexical, keeping max score and
        # unioning channel tags
    ...
    def _rerank(self, merged: dict[str, dict[str, Any]]) -> None:
        ...
        Apply reranking boosts:
          - notes present in all 3 channels → xALL_CHANNEL_RERANK
