...
2. **git remote** — ``git remote get-url origin`` parsed from the nearest
   ``.git`` directory found by walking up from the backend dir.  Works when
   the vault root *is* the git repo.  The owner/repo are extracted from the
   GitHub URL.
...
def resolve_upstream(backend_dir: str | None = None) -> tuple[str, str]:
    ...
    if git_root is not None:
        try:
            ...
            pass  # git not available or hung — fall through to error
    ...
    raise UpstreamIdentityError(
        ...
        "Or ensure the vault root is a git repo with a GitHub 'origin' "
