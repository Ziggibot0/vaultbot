...
def gh(
    ...
    if r.returncode != 0:
        raise GhError(
            f"gh {' '.join(args)} failed (exit {r.returncode}): {r.stderr.strip()[:500]}",
            stderr=r.stderr,
        )
    return r
...
def gh_api(
    ...
    if r.returncode != 0:
        raise GhError(
            f"gh api {method} {path} failed (exit {r.returncode}): {r.stderr.strip()[:500]}",
            stderr=r.stderr,
        )
...
def gh_raw(owner: str, repo: str, ref: str, path: str, timeout: int = 30) -> str:
    ...
    if r.returncode != 0:
        raise GhError(
            f"gh raw {owner}/{repo}/{path}@{ref} failed (exit {r.returncode}): {r.stderr.strip()[:500]}",
            stderr=r.stderr,
        )
    return r.stdout
