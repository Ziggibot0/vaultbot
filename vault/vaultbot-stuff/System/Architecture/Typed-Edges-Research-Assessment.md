...
- Their solution is a cloud service that handles maintenance — not applicable to the operator's goal of a local, accessible system

This directly conflicts with the operator's goal: a system where a 30B local model works from day 1 with minimal LLM usage. Typed edges add vocabulary discipline, syntax overhead, and ongoing gardening to every note write — forever.
